...
class SongPermissionTests(APITestCase):
    def setUp(self):
        # 1. Create two users
        self.user_owner = User.objects.create_user(username="testuser", email='owner@example.com', password='password123')
        self.user_hacker = User.objects.create_user(username="hacker", email='hacker@example.com', password='password123')
